...
SELECT
  ...
  u.email,
  CASE
    WHEN u.email = 'nobru05@hotmail.com' THEN 'admin'
    ELSE 'user'
  END as role,
  CASE
    WHEN u.email = 'nobru05@hotmail.com' THEN 'approved'
    ELSE 'pending'
  END as status
FROM auth.users u
...
-- Garante que nobru05@hotmail.com é admin
UPDATE profiles SET role = 'admin', status = 'approved'
WHERE email = 'nobru05@hotmail.com';
